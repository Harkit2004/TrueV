 ...
 */
/** @type {null | ((w: number, h: number) => import('canvas').Canvas)} */
let _headlessCreateCanvas = null;
...
export async function exportLive2DProject(project, images, opts = {}) {
  ...
  _headlessCreateCanvas = headlessCreateCanvas;
...
async function renderPartToCanvasPngTransformed(img, srcW, srcH, canvasW, canvasH, wm) {
  let canvas;
  if (_headlessCreateCanvas) {
    canvas = _headlessCreateCanvas(canvasW, canvasH);
  ...
  if (_headlessCreateCanvas && canvas.toBuffer) {
